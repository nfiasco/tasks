...
export function sumPublishedPoints(questions: Question[]): number {
    const filterd: Question[] = questions.filter(
        (Q: Question): boolean => Q.published,
    );
    return filterd.reduce(
        (total: number, Q: Question): number => total + Q.points,
        0,
    );
}
...
export function changeQuestionTypeById(
    ...
    return questions.map((Q: Question): Question => {
        if (Q.id === targetId) {
            ...
            return { ...Q, type: newQuestionType, options: newOption };
        } else {
            return Q;
        }
    });
...
export function duplicateQuestionInArray(
    ...
    return questions.reduce((result: Question[], q: Question): Question[] => {
        result.push(q);
        if (q.id === targetId) {
